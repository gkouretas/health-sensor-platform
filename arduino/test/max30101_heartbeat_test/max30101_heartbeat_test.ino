...
void setup()
{
  ...
  byte ledBrightness = 0x7F; //Options: 0-255 (0x00 - 0xFF)
  byte sampleAverage = 8; //Options: 1, 2, 4, 8, 16, 32
  byte ledMode = 3; // Options: 1 = Red only, 2 = Red + IR, 3 = Red + IR + Green, 4 = Green only
  ...
  //Take an average of [green] readings at power up
  ...
  //Pre-populate the plotter so that the Y scale is close to green values
  for (int x = 0 ; x < 500 ; x++)
