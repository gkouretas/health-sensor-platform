/***************************************************
  This is a library written for the Maxim MAX30101 Optical Smoke Detector
...
boolean MAX30101::begin(TwoWire &wirePort, uint32_t i2cSpeed, uint8_t i2caddr) {
  ...
  if (readPartID() != MAX_30101_EXPECTEDPARTID) {
...
uint8_t MAX30101::getINT1(void) {
  return (readRegister8(_i2caddr, MAX30101_INTSTAT1));
}
uint8_t MAX30101::getINT2(void) {
  return (readRegister8(_i2caddr, MAX30101_INTSTAT2));
}

void MAX30101::enableAFULL(void) {
  bitMask(MAX30101_INTENABLE1, MAX30101_INT_A_FULL_MASK, MAX30101_INT_A_FULL_ENABLE);
}
void MAX30101::disableAFULL(void) {
  bitMask(MAX30101_INTENABLE1, MAX30101_INT_A_FULL_MASK, MAX30101_INT_A_FULL_DISABLE);
}

void MAX30101::enableDATARDY(void) {
  bitMask(MAX30101_INTENABLE1, MAX30101_INT_DATA_RDY_MASK, MAX30101_INT_DATA_RDY_ENABLE);
}
void MAX30101::disableDATARDY(void) {
  bitMask(MAX30101_INTENABLE1, MAX30101_INT_DATA_RDY_MASK, MAX30101_INT_DATA_RDY_DISABLE);
}

void MAX30101::enableALCOVF(void) {
  bitMask(MAX30101_INTENABLE1, MAX30101_INT_ALC_OVF_MASK, MAX30101_INT_ALC_OVF_ENABLE);
}
void MAX30101::disableALCOVF(void) {
  bitMask(MAX30101_INTENABLE1, MAX30101_INT_ALC_OVF_MASK, MAX30101_INT_ALC_OVF_DISABLE);
}

void MAX30101::enablePROXINT(void) {
  bitMask(MAX30101_INTENABLE1, MAX30101_INT_PROX_INT_MASK, MAX30101_INT_PROX_INT_ENABLE);
}
void MAX30101::disablePROXINT(void) {
  bitMask(MAX30101_INTENABLE1, MAX30101_INT_PROX_INT_MASK, MAX30101_INT_PROX_INT_DISABLE);
}

void MAX30101::enableDIETEMPRDY(void) {
  bitMask(MAX30101_INTENABLE2, MAX30101_INT_DIE_TEMP_RDY_MASK, MAX30101_INT_DIE_TEMP_RDY_ENABLE);
}
void MAX30101::disableDIETEMPRDY(void) {
  bitMask(MAX30101_INTENABLE2, MAX30101_INT_DIE_TEMP_RDY_MASK, MAX30101_INT_DIE_TEMP_RDY_DISABLE);
}
...
void MAX30101::softReset(void) {
  ...
  {
    uint8_t response = readRegister8(_i2caddr, MAX30101_MODECONFIG);
...
void MAX30101::setPulseAmplitudeRed(uint8_t amplitude) {
  writeRegister8(_i2caddr, MAX30101_LED1_PULSEAMP, amplitude);
}

void MAX30101::setPulseAmplitudeIR(uint8_t amplitude) {
  writeRegister8(_i2caddr, MAX30101_LED2_PULSEAMP, amplitude);
}

void MAX30101::setPulseAmplitudeGreen(uint8_t amplitude) {
  writeRegister8(_i2caddr, MAX30101_LED3_PULSEAMP, amplitude);
}

void MAX30101::setPulseAmplitudeProximity(uint8_t amplitude) {
  writeRegister8(_i2caddr, MAX30101_LED_PROX_AMP, amplitude);
}
...
void MAX30101::setProximityThreshold(uint8_t threshMSB) {
  ...
  writeRegister8(_i2caddr, MAX30101_PROXINTTHRESH, threshMSB);
}
...
void MAX30101::disableSlots(void) {
  writeRegister8(_i2caddr, MAX30101_MULTILEDCONFIG1, 0);
  writeRegister8(_i2caddr, MAX30101_MULTILEDCONFIG2, 0);
}
...
void MAX30101::clearFIFO(void) {
  writeRegister8(_i2caddr, MAX30101_FIFOWRITEPTR, 0);
  writeRegister8(_i2caddr, MAX30101_FIFOOVERFLOW, 0);
  writeRegister8(_i2caddr, MAX30101_FIFOREADPTR, 0);
}

//Enable roll over if FIFO over flows
void MAX30101::enableFIFORollover(void) {
  bitMask(MAX30101_FIFOCONFIG, MAX30101_ROLLOVER_MASK, MAX30101_ROLLOVER_ENABLE);
}

//Disable roll over if FIFO over flows
void MAX30101::disableFIFORollover(void) {
  bitMask(MAX30101_FIFOCONFIG, MAX30101_ROLLOVER_MASK, MAX30101_ROLLOVER_DISABLE);
}
...
uint8_t MAX30101::getWritePointer(void) {
  return (readRegister8(_i2caddr, MAX30101_FIFOWRITEPTR));
}

//Read the FIFO Read Pointer
uint8_t MAX30101::getReadPointer(void) {
  return (readRegister8(_i2caddr, MAX30101_FIFOREADPTR));
}
...
float MAX30101::readTemperature() {
  ...
  writeRegister8(_i2caddr, MAX30101_DIETEMPCONFIG, 0x01);
  ...
  while (millis() - startTime < 100)
  {
    //uint8_t response = readRegister8(_i2caddr, MAX30101_DIETEMPCONFIG); //Original way
    //if ((response & 0x01) == 0) break; //We're done!
    
	//Check to see if DIE_TEMP_RDY interrupt is set
	uint8_t response = readRegister8(_i2caddr, MAX30101_INTSTAT2);
  ...
  int8_t tempInt = readRegister8(_i2caddr, MAX30101_DIETEMPINT);
  uint8_t tempFrac = readRegister8(_i2caddr, MAX30101_DIETEMPFRAC); //Causes the clearing of the DIE_TEMP_RDY interrupt
...
void MAX30101::setPROXINTTHRESH(uint8_t val) {
  writeRegister8(_i2caddr, MAX30101_PROXINTTHRESH, val);
}
...
uint8_t MAX30101::readPartID() {
  return readRegister8(_i2caddr, MAX30101_PARTID);
}

void MAX30101::readRevisionID() {
  revisionID = readRegister8(_i2caddr, MAX30101_REVISIONID);
}
...
void MAX30101::setup(byte powerLevel, byte sampleAverage, byte ledMode, int sampleRate, int pulseWidth, int adcRange) {
  softReset(); //Reset all configuration, threshold, and data registers to POR values

  //FIFO Configuration
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  //The chip will average multiple samples of same type together if you wish
  if (sampleAverage == 1) setFIFOAverage(MAX30101_SAMPLEAVG_1); //No averaging per FIFO record
  else if (sampleAverage == 2) setFIFOAverage(MAX30101_SAMPLEAVG_2);
  else if (sampleAverage == 4) setFIFOAverage(MAX30101_SAMPLEAVG_4);
  else if (sampleAverage == 8) setFIFOAverage(MAX30101_SAMPLEAVG_8);
  else if (sampleAverage == 16) setFIFOAverage(MAX30101_SAMPLEAVG_16);
  else if (sampleAverage == 32) setFIFOAverage(MAX30101_SAMPLEAVG_32);
  else setFIFOAverage(MAX30101_SAMPLEAVG_4);
...
//   if (ledMode == 3) setLEDMode(MAX30101_MODE_MULTILED); //Watch all three LED channels
//   else if (ledMode == 2) setLEDMode(MAX30101_MODE_REDIRONLY); //Red and IR
//   else 
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

  //Particle Sensing Configuration
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  if(adcRange < 4096) setADCRange(MAX30101_ADCRANGE_2048); //7.81pA per LSB
  else if(adcRange < 8192) setADCRange(MAX30101_ADCRANGE_4096); //15.63pA per LSB
  else if(adcRange < 16384) setADCRange(MAX30101_ADCRANGE_8192); //31.25pA per LSB
  else if(adcRange == 16384) setADCRange(MAX30101_ADCRANGE_16384); //62.5pA per LSB
  else setADCRange(MAX30101_ADCRANGE_2048);

  if (sampleRate < 100) setSampleRate(MAX30101_SAMPLERATE_50); //Take 50 samples per second
  else if (sampleRate < 200) setSampleRate(MAX30101_SAMPLERATE_100);
  else if (sampleRate < 400) setSampleRate(MAX30101_SAMPLERATE_200);
  else if (sampleRate < 800) setSampleRate(MAX30101_SAMPLERATE_400);
  else if (sampleRate < 1000) setSampleRate(MAX30101_SAMPLERATE_800);
  else if (sampleRate < 1600) setSampleRate(MAX30101_SAMPLERATE_1000);
  else if (sampleRate < 3200) setSampleRate(MAX30101_SAMPLERATE_1600);
  else if (sampleRate == 3200) setSampleRate(MAX30101_SAMPLERATE_3200);
  else setSampleRate(MAX30101_SAMPLERATE_50);

  //The longer the pulse width the longer range of detection you'll have
  //At 69us and 0.4mA it's about 2 inches
  //At 411us and 0.4mA it's about 6 inches
  if (pulseWidth < 118) setPulseWidth(MAX30101_PULSEWIDTH_69); //Page 26, Gets us 15 bit resolution
  else if (pulseWidth < 215) setPulseWidth(MAX30101_PULSEWIDTH_118); //16 bit resolution
  else if (pulseWidth < 411) setPulseWidth(MAX30101_PULSEWIDTH_215); //17 bit resolution
  else if (pulseWidth == 411) setPulseWidth(MAX30101_PULSEWIDTH_411); //18 bit resolution
  else setPulseWidth(MAX30101_PULSEWIDTH_69);
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  ...
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  if (ledMode == 1) enableSlot(1, SLOT_RED_LED);
  if (ledMode == 2) enableSlot(2, SLOT_IR_LED);
  if (ledMode == 3) enableSlot(3, SLOT_GREEN_LED);
  if (ledMode == 4) enableSlot(1, SLOT_GREEN_LED);
...
uint16_t MAX30101::check(void)
{
  ...
  if (readPointer != writePointer)
  {
    ...
    _i2cPort->beginTransmission(MAX30101_ADDRESS);
    _i2cPort->write(MAX30101_FIFODATA);
    ...
    while (bytesLeftToRead > 0)
    {
      ...
      _i2cPort->requestFrom(MAX30101_ADDRESS, toGet);
...
//
uint8_t MAX30101::readRegister8(uint8_t address, uint8_t reg) {
...
void MAX30101::writeRegister8(uint8_t address, uint8_t reg, uint8_t value) {
  _i2cPort->beginTransmission(address);
  _i2cPort->write(reg);
